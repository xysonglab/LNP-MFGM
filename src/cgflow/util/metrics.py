...
class AverageStrainEnergy(GenerativeMetric):
    ...
    def update(self, mols: list[Chem.rdchem.Mol]) -> None:
        ...
        valid_indices, valid_energies = tuple(zip(*valids, strict=False))
        original_energies = [smolRD.calc_energy(mols[idx], per_atom=self.per_atom) for idx in valid_indices]
        energy_diffs = [orig - opt for orig, opt in zip(original_energies, valid_energies, strict=False)]
...
class AverageOptRmsd(GenerativeMetric):
    ...
    def update(self, mols: list[Chem.rdchem.Mol]) -> None:
        ...
        valid_indices, opt_mols = tuple(zip(*valids, strict=False))
        original_mols = [mols[idx] for idx in valid_indices]
        rmsds = [smolRD.conf_distance(mol1, mol2) for mol1, mol2 in zip(original_mols, opt_mols, strict=False)]
...
class MolecularAccuracy(PairMetric):
    ...
    def update(self, predicted: list[Chem.rdchem.Mol], actual: list[Chem.rdchem.Mol]) -> None:
        ...
        matches = [pred == act for pred, act in zip(predicted_smiles, actual_smiles, strict=False) if act is not None]
...
class MolecularPairRMSD(PairMetric):
    ...
    def update(self, predicted: list[Chem.rdchem.Mol], actual: list[Chem.rdchem.Mol]) -> None:
        ...
        rmsds = [smolRD.conf_distance(pred, act, align=self.align) for pred, act in valid_pairs]
