    ...
    def __call__(self, df):
        ...
        with torch.no_grad():
            ...
            edge_s, edge_v = _edge_features(
                coords, edge_index, D_max=self.edge_cutoff, num_rbf=self.num_rbf, device=self.device
            )
...
class BaseModel(nn.Module):
    ...
    def __init__(self, num_rbf=16):
        ...
        self.W_e = nn.Sequential(
            LayerNorm((num_rbf, 1)), GVP((num_rbf, 1), _DEFAULT_E_DIM, activations=(None, None), vector_gate=True)
        )
        ...
        self.layers = nn.ModuleList(
            GVPConvLayer(_DEFAULT_V_DIM, _DEFAULT_E_DIM, activations=activations, vector_gate=True) for _ in range(5)
        )

        ns, _ = _DEFAULT_V_DIM
        self.W_out = nn.Sequential(
            LayerNorm(_DEFAULT_V_DIM), GVP(_DEFAULT_V_DIM, (ns, 0), activations=activations, vector_gate=True)
        )
...
class SMPTransform(BaseTransform):
    ...
    def __call__(self, elem):
        ...
        with torch.no_grad():
            data.label = torch.as_tensor(elem["labels"], device=self.device, dtype=torch.float32)
        return data
...
class PPIModel(BaseModel):
    ...
    def __init__(self, **kwargs):
        ...
        ns, _ = _DEFAULT_V_DIM
        self.dense = nn.Sequential(
            nn.Linear(2 * ns, 4 * ns), nn.ReLU(inplace=True), nn.Dropout(p=0.1), nn.Linear(4 * ns, 1)
        )
...
class RESModel(BaseModel):
    ...
    def __init__(self, **kwargs):
        ...
        ns, _ = _DEFAULT_V_DIM
        self.dense = nn.Sequential(
            nn.Linear(ns, 2 * ns), nn.ReLU(inplace=True), nn.Dropout(p=0.1), nn.Linear(2 * ns, 20)
        )
